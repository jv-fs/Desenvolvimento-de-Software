...
class Instruments:
    ...
    instruments_to_id = {nome: identificador for identificador, nome in id_to_instruments.items()}

    @classmethod
    def getInstrumentFromName(cls, name: str):
        return cls.instruments_to_id.get(name, None)

    @classmethod
    def getInstrumentFromNumber(cls, number: int):
        return cls.id_to_instruments.get(number, None)

class Notes:
    base_notes = ["C", "DB", "D", "MB", "E", "F", "GB", "G", "AB", "A", "H", "B"]

    @classmethod
    def getNoteFromName(cls, note_name: str, octave = None):

        if octave is None:
            octave = VoiceConstants.INITIAL_OCTAVE

        upper_note_name = note_name.upper()

        try:
            index = cls.base_notes.index(upper_note_name)
            midi_code = (octave + 1) * 12 + index
            return midi_code
